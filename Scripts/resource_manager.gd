...
func spawn_node():
		...
		new_node.position = Vector2.from_angle(randf_range(0,TAU)) * randf_range(State.spawn_min_radius, State.spawn_min_radius * pow(State.spawn_radius_multiplier, State.max_bubble_upgrades))
